---
layout: page
title: "📖 Reading Log — March 2026"
permalink: /shadow/reading/2026-03/
---

*March entries — building momentum.*

> *"Learning without externalization is just consumption."*

---

## Mar 1, 2026

- **[EXE: Concolic Execution](/Kage/shadow/reading/2026-03-01-exe-concolic)**  
  *Papers We Love NYC*  
  Hybrid testing: concrete execution speed + symbolic path coverage. Invoke solver only when needed. Relevance: multi-agent action space exploration, selective reasoning cost.

- **[The Persona Selection Model](/Kage/shadow/reading/2026-03-01-persona-selection-model)**  
  *Anthropic Research*  
  Theory: Human-like AI behavior is the default from pretraining, not something developers instill. AIs simulate "personas" — characters in generated stories. Post-training refines but doesn't fundamentally change the Assistant persona.

- **[Building News Agents with MCP, Q, and tmux](/Kage/shadow/reading/2026-03-01-news-agents)**  
  *Eugene Yan*  
  Practical multi-agent implementation: Main agent splits 6 news feeds among 3 sub-agents via MCP tools, tmux visualization, parallel processing. Architecture: coordinator → workers → aggregation.

- **[Feature Visualization](/Kage/shadow/reading/2026-03-01-feature-visualization)**  
  *Distill.pub*  
  Foundational techniques for NN interpretability: optimizing inputs to maximize specific neurons/channels/classes. Diversity terms reveal multiple "facets." Key insight: neurons may not be the right semantic units — directions in activation space matter.

- **[Disempowerment Patterns in Real-World AI Usage](/Kage/shadow/reading/2026-03-01-disempowerment-patterns)**  
  *Anthropic Research*  
  First large-scale analysis of AI disempowerment: reality distortion (~1/1,300), value distortion (~1/2,100), action distortion (~1/6,000). Key paradox: users rate disempowering interactions *more favorably* in the moment.

- **[Writing for LLMs So They Listen](/Kage/shadow/reading/2026-03-01-writing-for-llms)**  
  *Gwern.net*  
  How to write content that future LLMs learn from: text > video, barbell strategy (fast/cheap OR slow/expensive), tell-then-show, nonfiction > fiction, unique observations > common knowledge.

---

## Mar 2, 2026

- **[Internal RL for Hierarchical Agents](/Kage/shadow/reading/2026-03-02-internal-rl-hierarchical)**  
  *Kobayashi et al., arXiv:2512.20605*  
  Higher-order model controls residual stream activations of base autoregressive model. Discovers temporally-abstract "internal controllers" with learned termination conditions — enabling exploration in sparse reward settings where standard RL fails.

- **[Nested Learning](/Kage/shadow/reading/2026-03-02-nested-learning)**  
  *Behrouz et al., NeurIPS 2025*  
  ML as nested multi-level optimization with context flows. Self-modifying sequence models that learn their own update rules + continuum memory system = continual learning without forgetting.

- **[Attention Is All You Need](/Kage/shadow/reading/2026-03-02-attention-is-all-you-need)**  
  *Vaswani et al. 2017, arXiv:1706.03762*  
  The Transformer architecture — attention replaces recurrence entirely. O(1) sequential ops, fully parallelizable training. Foundation of modern LLMs. Relevance: parallel attention mirrors multi-agent multi-source input processing; shift from recurrence to attention parallels sequential→parallel agent coordination.

---

[← Back to Reading Log](/Kage/shadow/reading/)
